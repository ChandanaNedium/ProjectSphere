...
export const SIMILARITY_WEIGHTS = {
  ...
  TECHNOLOGY: 0.2, // 20% Technology stack & frameworks
  DOMAIN: 0.1, // 10% Domain & subdomain alignment
  OUTCOME: 0.1, // 10% Target outcome & scope
} as const;
...
export const GLOBAL_SIMILARITY_CONFIG = {
  ...
  MAX_GITHUB_CANDIDATES: 12,
  MAX_DEEP_ANALYSIS_RESULTS: 4,
  MAX_REPOSITORY_FILES: 24,
  MAX_REPOSITORY_FILE_SIZE: 120000,
  REPOSITORY_CACHE_TTL_MS: 15 * 60 * 1000,
...
export function getEnvConfig() {
  return {
    ...
    ).toLowerCase() as "local" | "gemini" | "openai",
    geminiApiKey:
      process.env.GEMINI_API_KEY || process.env.EMBEDDING_API_KEY || "",
    openAiApiKey: process.env.OPENAI_API_KEY || "",
    webSearchProvider: (
      process.env.WEB_SEARCH_PROVIDER || "open_web"
    ).toLowerCase() as
      | "tavily"
      | "serpapi"
      | "bing"
