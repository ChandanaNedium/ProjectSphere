...
export async function runGlobalSimilarityCheck(
  ...
  const project = buildStructuredRepresentation(input)
  ...
  const sourceBreakdown: Record<SourceType, SourceStats> = {
    ProjectSphere: calculateSourceStats('ProjectSphere', rankedCandidates, rawProjectSphereCount, projectSphereResult.error),
    GitHub: calculateSourceStats('GitHub', rankedCandidates, rawGithubCount, githubResult.error || (githubResult.rateLimited ? 'Rate limited — showing cached results' : undefined)),
    Web: calculateSourceStats('Web', rankedCandidates, rawWebCount, webResult.error),
    'Research Paper': calculateSourceStats('Research Paper', rankedCandidates, rawResearchCount, researchResult.error),
  }

  // 10. Global High-Level Summary
  const highestCandidate = rankedCandidates[0]
  const highestMatchScore = highestCandidate ? highestCandidate.similarityScore : 0
  const highestMatchSource = highestCandidate ? highestCandidate.source : 'ProjectSphere'
  const highestMatchProjectTitle = highestCandidate ? highestCandidate.title : 'None found'
  ...
  return {
    ...
    sourcesAnalyzed: {
      ...
      total: rawProjectSphereCount + rawGithubCount + rawWebCount + rawResearchCount,
    },
    sourceBreakdown,
...
function calculateSourceStats(
  ...
  if (errorMessage && matches.length === 0) {
    status = errorMessage.includes('rate limit') ? 'rate_limited' : 'unavailable'
