...
export async function compareProjectWithCandidate(
  ...
  const candidateMethod = candidate.methodology || candidate.description
  const candidateProblem = candidate.problemStatement || candidate.description
  const candidateOutcome = candidate.expectedOutcome || candidate.description

  // 1. Prepare dimension text pairs
  const textPairs: Array<{ dimension: keyof SimilarityDimensionScores; textA: string; textB: string }> = [
    {
    ...
    },
    {
      ...
      textA: `Methodology and Architecture: ${project.methodology}`,
      textB: `Methodology and Architecture: ${candidateMethod}`,
    },
    {
      dimension: 'technologyScore',
      textA: `Technologies: ${project.technologies.join(', ')}`,
      textB: `Technologies: ${candidateTech.join(', ')}`,
    },
    {
  ...
  const candidateTechSet = new Set(candidateTech.map(t => t.toLowerCase()))
  let techJaccard = 0
  if (projectTechSet.size > 0 && candidateTechSet.size > 0) {
    const inter = new Set([...projectTechSet].filter(x => candidateTechSet.has(x)))
  ...
  const combinedTechScore = (rawScores['technologyScore'] || 0) * 0.6 + techJaccard * 0.4

  const problemScore = Math.min(100, Math.round((rawScores['problemScore'] || 0) * 100))
  const methodologyScore = Math.min(100, Math.round((rawScores['methodologyScore'] || 0) * 100))
  const technologyScore = Math.min(100, Math.round(combinedTechScore * 100))
  const domainScore = Math.min(100, Math.round((rawScores['domainScore'] || 0) * 100))
  const outcomeScore = Math.min(100, Math.round((rawScores['outcomeScore'] || 0) * 100))
  ...
  const { commonAreas, differences } = extractSimilaritiesAndDifferences(project, candidate)

  // 6. Generate Contextual Explanation
  const explanation = generateExplanation(project, candidate, finalScore, dimensionScores, commonAreas)
...
function extractSimilaritiesAndDifferences(
  ...
  const projectTokens = new Set(tokenizeText(`${project.title} ${project.problem} ${project.methodology}`))
  const candidateTokens = new Set(tokenizeText(`${candidate.title} ${candidate.description}`))

  // Common concepts
  const sharedTokens = [...projectTokens].filter(t => candidateTokens.has(t) && t.length > 3)
  ...
  if (sharedTech.length > 0) {
    commonAreas.push(`Shared stack: ${sharedTech.slice(0, 3).map(capitalizeWord).join(', ')}`)
  }
  ...
  if (uniqueProjectTech.length > 0) {
    differences.push(`Your unique stack: ${uniqueProjectTech.slice(0, 2).join(', ')}`)
  }
  ...
  if (uniqueCandidateTech.length > 0) {
    differences.push(`Target work relies on: ${uniqueCandidateTech.slice(0, 2).join(', ')}`)
  }
  ...
  if (differences.length === 0) {
    differences.push('Distinct implementation architecture', 'Differentiated dataset / target environment')
  }
