...
export function buildStructuredRepresentation(
  ...
  const problem =
    input.problemStatement?.trim() || extractProblem(input.description);
  const methodology =
    input.methodology?.trim() ||
    extractMethodology(input.description, technologies);
  const expectedOutcome =
    input.expectedOutcome?.trim() || extractOutcome(input.description);
  ...
  const repositoryProfile = input.repositoryProfile;
  const implementationConcepts =
    repositoryProfile?.implementationConcepts || [];
  const architecture = repositoryProfile?.architecture.join(", ") || "";
  ...
  const rawNormalizedText = [
    ...
    `IMPLEMENTATION: ${implementationConcepts.join(", ")}`,
...
function extractProblem(desc: string): string {
  ...
  const problemSentences = sentences.filter((s) =>
    /problem|challenge|issue|lack|difficult|waste|cost|inefficient|delay|cheat|manual|threat|loss/i.test(
      s,
    ),
  );
...
function extractMethodology(desc: string, tech: string[]): string {
  ...
  const methodSentences = sentences.filter((s) =>
    /using|trained|model|algorithm|architecture|dataset|pipeline|network|protocol|system|framework|process/i.test(
      s,
    ),
  );
  if (methodSentences.length > 0)
    return methodSentences.slice(0, 2).join(". ") + ".";
...
function extractOutcome(desc: string): string {
  ...
  const outcomeSentences = sentences.filter((s) =>
    /achieve|reduce|improve|result|enable|provide|predict|detect|classify|deliver|optimize/i.test(
      s,
    ),
  );
  if (outcomeSentences.length > 0)
    return outcomeSentences.slice(0, 2).join(". ") + ".";
...
export function generateSearchQueries(
  ...
  const topTech = [...project.technologies, ...project.implementationConcepts]
    .slice(0, 5)
    .join(" ");
  ...
  queries.push({
    ...
    rationale:
      "Searches for direct conceptual and name matches across all repositories and literature.",
  });
  ...
  queries.push({
    ...
    rationale:
      "Identifies solutions addressing the same fundamental problem space.",
  });
  ...
  if (topTech) {
    ...
    queries.push({
      ...
      rationale:
        "Finds open-source projects and codebases built with identical tech stacks.",
    });
  }

  // 4. Academic Research / Methodology Query
  const methodKeywords = tokenizeText(project.methodology)
    .slice(0, 3)
    .join(" ");
  const academicQuery =
    `${project.domain} ${cleanTitle} ${methodKeywords}`.slice(0, 90);
  queries.push({
    query: academicQuery.trim(),
    perspective: "research_literature",
    targetSources: ["Research Paper", "Web"],
    rationale:
      "Finds peer-reviewed publications and conference papers with similar methodologies.",
  });
