...
export async function searchGitHub(
  ...
  const relevantQueries = queries.filter((q) =>
    q.targetSources.includes("GitHub"),
  );
  ...
  for (const q of relevantQueries.slice(0, 2)) {
    try {
      ...
      if (response.status === 403 || response.status === 429) {
        isRateLimited = true;
        lastError =
          "GitHub API rate limit reached. Using available public repo metadata.";
        break;
      }
      ...
      for (const item of items) {
        ...
        resultsMap.set(item.html_url, {
          ...
          description:
            item.description ||
            `Public repository by ${item.owner?.login} with ${item.stargazers_count} stars.`,
          source: "GitHub",
  ...
  const results = Array.from(resultsMap.values()).slice(
    0,
    GLOBAL_SIMILARITY_CONFIG.MAX_GITHUB_CANDIDATES,
  );
  return {
    results,
